/**
 * Class Marker: Mengelola data dan representasi visual setiap titik di peta.
 */
class Marker {
...
/**
 * Class MapManager: Otak utama aplikasi GIS.
 */
class MapManager {
  constructor(mapId) {
    // Inisialisasi Peta (Pusat Yogyakarta)
    this.map = L.map(mapId).setView([-7.8013672, 110.3647568], 13);
    this.markerLayer = L.layerGroup().addTo(this.map);
    L.control
      .scale({
        position: "bottomleft", // Posisi di kanan bawah
        metric: true, // Menampilkan satuan meter/km
      })
      .addTo(this.map);
    // 1. Definisi Berbagai Layer Peta
    this.layers = {
      // Google Street View
    ...
    // 2. Tambahkan Kontrol Pemilih Layer ke Peta
  ...
  initListeners() {
    ...
    // Toggle manual tetap ada di UI jika dibutuhkan
